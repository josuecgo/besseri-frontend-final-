...
import { CUSTOMER_HOME_SCREEN_ROUTES, OrderStatusCode, showToaster } from '../../util/constants';
...
const RiderOrderDetail = props => {
    ...
    const getMyAddress = async () => {
        try {
          ...
          const apiCall = await axios.get(`${customer_api_urls.get_addresses}/${order?.user._id}`);
          setLoading(false);
          if (apiCall.status == api_statuses.success) {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            ...
            <View
                ...
                {order?.order_status_code == 'PARCEL_DELIVERED' ? (
                    <Text style={{ ...CommonStyles.fontFamily, color: 'black',fontSize:adjust(11) }}>
                                ...
                                <View
                                    ...
                                    <View>
                                        <Text style={{ fontSize: 16, fontWeight: 'bold' }}>
                                            {itemData.item.quantity * itemData.item.price} MXN
                                        </Text>
                                    </View>
                                </View>
                            </View>
                        ...
                        order.products.map( (itemData) => {
                            ...
                            return (
                                ...
                                <View
                                    ...
                                    />
                                    <Text
                                        style={{fontSize:adjust(11), fontWeight: 'bold', paddingLeft: 5 }}>
                                        {itemData.name}
                                    </Text>
                                </View>
                                <View
