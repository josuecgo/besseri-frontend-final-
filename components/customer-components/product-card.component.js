...
const ProductCardComponent = ({comision,cartProduct,inCart,data,onAddToCart,increaseQuantity,decreaseQuantity,horizontal,onRemoveFromCart,onViewDetail}) => {
  ...
  if(horizontal) {
    const horizontalCardStyles = {
      cardContainer: {
        width: '100%', padding: 20, elevation: 12, borderBottomWidth: 0.3,backgroundColor:Colors.white,elevation:2
      },
      img: {
        width: 50, height: 60, borderWidth: 1, borderColor: 'transparent', borderRadius: 10
      },
      wrapper: {
    ...
    <View style={horizontalCardStyles.wrapper}>
      <View>
        ...
        <Text style={{color:'grey',...CommonStyles.fontFamily}}>
          {moneda((Number(data?.price) + Number( comision * data?.price / 100)))} MXN
        </Text>
        ...
        onPress={onAddToCart}
        >
          <Text style={{...CommonStyles.fontFamily,color:Colors.brightBlue,marginTop:10}}>
            {cartProductIds?.includes(data?._id) ? 'QUITAR DEL CARRITO' : 'AÑADIR AL CARRITO'}
            </Text>
        </TouchableOpacity>
    ...
    />
      ...
      >
        {/* <View style={{ backgroundColor:'red' }} > */}
          {/* < Text style={{marginVertical:3,colo}}>Besseri Autoparts</Text> */}
          <Text style={styles.productTitle}>{data?.name}</Text>
          <View style={{ flexDirection: 'row', justifyContent: 'space-between',alignItems:'baseline' }} >
            <View>
              <Text style={styles.productPrice}>{moneda((Number(data?.price) + Number( comision * data?.price / 100)))}</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  quantityCard: { width: '100%', height: 40, marginTop: 10, flexDirection: 'row', borderWidth: 3, borderColor: '#D3D3D3' },
