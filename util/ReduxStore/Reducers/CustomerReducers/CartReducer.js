    ...
    switch (action.type) {
        ...
        case ADD_ITEM_TO_CART:
            ...
            if (existVendorIndex !== -1) {
                ...
                const newCartSeller = {
                    cart_items: [{ ...action.data, quantity: 1 }],
        ...
        case DELETE_ITEM:
            const selectedItem = state.cart_items.find(item => item?._id == action.data);

            if (!selectedItem) return state; // Si no encuentra el producto, no hace nada

            const productAmount = Number(selectedItem.price) * selectedItem.quantity;
            ...
            return {
                ...
                businessId: state.cart_items.length === 1 ? null : state.businessId,
                carts_by_seller: state.carts_by_seller
                    .map(cart => {
                        if (cart.cart_items.some(i => i._id === action.data)) {
                            const updatedCartItems = cart.cart_items.filter(i => i._id != action.data);
                            const updatedTotal = updatedCartItems.reduce((sum, item) => sum + (item.price * item.quantity), 0);

                            return updatedCartItems.length > 0
                                ? { ...cart, cart_items: updatedCartItems, cart_items_ids: updatedCartItems.map(i => i._id), total_amount: updatedTotal }
                                : null; // Si el carrito queda vacío, eliminarlo
                        }
                        return cart;
                    })
                    .filter(cart => cart !== null) // Filtrar carritos vacíos
            };
        case DESCUENTO:
        ...
        case INCREASE_QUANTITY:
            return {
                ...
                carts_by_seller: state.carts_by_seller.map(seller => ({
                    ...seller,
                    cart_items: seller.cart_items.map(product =>
                        product._id === action.data
                            ? { ...product, quantity: product.quantity + 1 }
                            : product
                    ),
                    total_amount: seller.cart_items.some(p => p._id === action.data)
                        ? seller.total_amount + Number(action.price)
                        : seller.total_amount,
                })),
            };
